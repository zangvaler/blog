<template>
  <div class="login">
    <div class="padding-all">
		<div class="header">
			<h1>Zangval With Blog</h1>
		</div>

		<div class="design-w3l">
			<div class="mail-form-agile">
					<input type="text" name="name" placeholder="User Name" v-model="userName"/>
					<input type="password"  name="password" class="padding" placeholder="Password" v-model="passWord"/>
          <p class="menu">
            <span class="forgetPwd">忘记密码</span>
            <span class="createUser">账号注册</span>
          </p>
					<input type="submit" value="login" @click="logininif">
			</div>
		  <div class="clear"> </div>
		</div>
		<div class="footer">
  ...
  methods: {
    logininif(){
      ...
      this.$http.post('http://localhost:3000/users/login',{
        ...
        let code = reslut.data.result.code
        let name = reslut.data.result.username
        ...
        }else if(reslut.data.result.code == 2){
          this.$router.push({ path:'./creatUser/addUsers'})
        }
      })
...
.forgetPwd:hover,.createUser:hover{
  color: #03A9F4;
}
.padding {
margin: 20px 0 0px;
